...
Hooks.once('tokenActionHudCoreApiReady', async (coreModule) => {
    ...
    Utils = class Utils {
        ...
        static getAttributeConditionsMap() {
            ...
            for (const [attributeKey, conditionData] of Object.entries(conditionEffects)) {
            ...
            try {
                switch (statType) {
                    case 'hitpoints': {
                        ...
                        if (damage > 0) {
                            content = game.i18n.format('tokenActionHud.dragonbane.chat.stats.hitpointsDamaged', {
                                name: actorName, current, max, damage
                            }) || `<strong>${actorName}</strong> has ${current}/${max} HP (${damage} damage taken).`
                        } else {
                            content = game.i18n.format('tokenActionHud.dragonbane.chat.stats.hitpoints', {
                                name: actorName, current, max
                    ...
                    case 'willpoints': {
                        ...
                        if (spent > 0) {
                            content = game.i18n.format('tokenActionHud.dragonbane.chat.stats.willpointsSpent', {
                                name: actorName, current, max, spent
                            }) || `<strong>${actorName}</strong> has ${current}/${max} WP (${spent} WP spent).`
                        } else {
                            content = game.i18n.format('tokenActionHud.dragonbane.chat.stats.willpoints', {
                                name: actorName, current, max
                    ...
                    case 'movement': {
                        ...
                        content = game.i18n.format('tokenActionHud.dragonbane.chat.stats.movement', {
                            name: actorName, movement
                    ...
                    case 'encumbrance': {
                        ...
                        let baseContent = game.i18n.format('tokenActionHud.dragonbane.chat.stats.encumbrance', {
                            name: actorName, current, max
                        }) || `<strong>${actorName}</strong> is carrying ${current}/${max} items.`

                        if (overEncumbered) {
                            const overNote = game.i18n.localize('tokenActionHud.dragonbane.chat.stats.overEncumbered') || 'Over-encumbered!'
                    ...
                    case 'ferocity': {
                        const ferocity = actor.system.ferocity.value || actor.system.ferocity
                        content = game.i18n.format('tokenActionHud.dragonbane.chat.stats.ferocity', {
                            name: actorName, ferocity
                ...
                console.error('Token Action HUD Dragonbane: Error creating stats chat message:', error)
        ...
        static getSpellSkillValue(actor, spell) {
            ...
            if (schoolName.toLowerCase() === 'general' || schoolName.toLowerCase() === 'dod.spell.general') {
        ...
        static parseMonsterAttackName(tableResult) {
            if (!tableResult) return { attackName: 'Unknown Attack', cleanDescription: '' }
         ...
         * @returns {string} HTML content for chat message
         */
        static createMonsterAttackChatContent(tableResult, attackTable, roll = null) {
            const { attackName, cleanDescription } = this.parseMonsterAttackName(tableResult)
