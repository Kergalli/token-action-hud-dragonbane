    ...
    return {
        ...
        buildStats: async function () {
            ...
            const statConfigs = [
                ...
                {
                    id: 'willpoints',
                    condition: () => this.actor.type !== 'monster' && this.actor.system?.willPoints?.max > 0,
                    getName: () => {
                ...
                {
                    id: 'encumbrance',
                    condition: () => this.actor.type === 'character' && this.actor.system?.encumbrance,
                    getName: () => {
                ...
                {
                    id: 'ferocity',
                    condition: () => this.actor.type === 'monster' && this.actor.system?.ferocity !== undefined,
                    getName: () => {
            ...
            for (const config of statConfigs) {
                if (config.condition()) {
                    ...
                    actions.push(this._createSimpleAction(
                        config.id,
                        name,
                        config.img,
                        'stats',
        ...
        buildTraits: async function () {
            ...
            if ((this.actorType !== 'npc' && this.actorType !== 'monster') || !this.actor?.system?.traits?.trim()) {
                return
            }
        ...
        buildWeapons: async function () {
            ...
            const weaponNameFormatter = (itemData) => {
                ...
                if (!this.showEquippedWeaponsOnly && this.actor.type === 'character') {
                    ...
                    if (isEquipped) {
                        name = game.i18n.format('tokenActionHud.dragonbane.actions.weapon.equipped', { name })
                            || `⚔ ${name}`
                    }
                }
            ...
            const actions = [...weapons].map(([itemId, itemData]) => {
                ...
                return this._createItemAction(itemId, name, itemData, 'weapon', 'handleWeaponAction', extraProps)
            })
        ...
        buildSpells: async function () {
            ...
            for (const [itemId, itemData] of this.items) {
                ...
                if (this.actor.type === 'character' && !this.showAllSpells && !isPrepared) continue
            ...
            const spellNameFormatter = (itemData) => {
                ...
                if (this.showAllSpells && this.actor.type === 'character') {
                    const isPrepared = itemData.system?.memorized === true
                    if (isPrepared) {
                        name = game.i18n.format('tokenActionHud.dragonbane.actions.spell.prepared', { name })
                            || `⚡ ${name}`
                    }
                }
                return name
            ...
            const rankGroups = ['spellRank0', 'spellRank1', 'spellRank2', 'spellRank3']
            ...
            for (const [rank, spells] of spellsByRank) {
                ...
                const actions = [...spells].map(([itemId, itemData]) => {
                    ...
                    return this._createItemAction(itemId, name, itemData, 'spell', 'handleSpellAction')
                })
        ...
        buildSkills: async function () {
            ...
            if (this.actor.system.coreSkills?.length > 0) {
                this.buildSkillCategory(this.actor.system.coreSkills, 'coreSkills', actionTypeId)
            }

            // Build Weapon Skills (if setting enabled)
            if (this.actor.system.weaponSkills?.length > 0 && this.showWeaponSkills) {
                this.buildSkillCategory(this.actor.system.weaponSkills, 'weaponSkills', actionTypeId)
            }

            // Build Secondary Skills (if setting enabled)
            if (this.actor.system.secondarySkills?.length > 0 && this.showSecondarySkills) {
                this.buildSkillCategory(this.actor.system.secondarySkills, 'secondarySkills', actionTypeId)
            }
        },
        ...
        buildAttributes: async function () {
            ...
            const attributeConfigs = Object.entries(ATTRIBUTES).map(([attrKey, attrConfig]) => ({
                id: attrKey,
                name: () => {
        ...
        buildInventory: async function () {
            ...
            for (const type of inventoryTypes) {
                const items = this._collectItemsWithEquipmentFilter(type, this.showUnequippedItems, false)
                if (items.size > 0) {
            ...
            for (const [type, typeMap] of inventoryMap) {
                ...
                const actions = [...typeMap].map(([itemId, itemData]) => {
                    ...
                    return this._createItemAction(itemId, displayName, itemData, 'item', 'handleItemAction')
                })
        ...
        buildInjuries: async function () {
            ...
            const actions = [...injuries].map(([itemId, itemData]) => {
                return this._createItemAction(itemId, itemData.name, itemData, 'item', 'handleConditionAction')
            })
        ...
        buildCombatActions: async function () {
            ...
            const getSkillValueByName = (skillName) => {
                if (!this.actor?.system?.coreSkills) return 0
                const skill = this.actor.system.coreSkills.find(s => s.name === skillName)
            ...
            const combatConfigs = [
                {
                    id: 'dodge',
                    condition: () => this.actorType === 'character' || this.actorType === 'npc',
                    name: () => {
                        const baseName = coreModule.api.Utils.i18n('tokenActionHud.dragonbane.dodge')
                ...
                },
                {
                    ...
                    condition: () => this.actorType === 'character' || this.actorType === 'npc',
                    name: () => {
                        const baseName = coreModule.api.Utils.i18n('tokenActionHud.dragonbane.firstAid')
                ...
                },
                {
                    ...
                    condition: () => this.actorType === 'character' || this.actorType === 'npc',
                    name: () => {
                        const baseName = coreModule.api.Utils.i18n('tokenActionHud.dragonbane.rallyOther')
                ...
                },
                {
                    ...
                    name: () => {
                        const baseName = coreModule.api.Utils.i18n('tokenActionHud.dragonbane.rallySelf')
            ...
            if (this.actorType === 'character' && this.showDeathRoll && Utils.needsDeathRoll(this.actor)) {
                const deathRolls = Utils.getDeathRollStatus(this.actor)
                const deathRollText = coreModule.api.Utils.i18n('tokenActionHud.dragonbane.deathRollStatus')
                    ?.replace('{successes}', deathRolls.successes)
                    ?.replace('{failures}', deathRolls.failures) || `Death Roll (${deathRolls.successes}/${deathRolls.failures})`
        ...
        buildMonsterAttacks: async function () {
            ...
            try {
                ...
                for (let result of attackTable.results) {
                    ...
                    const attackText = coreModule.api.Utils.i18n('tokenActionHud.dragonbane.attackNumber')
                        ?.replace('{index}', attackIndex)
                        ?.replace('{name}', attackName) || `Attack ${attackIndex}: ${attackName}`
        ...
        buildMonsterWeaponDamage: async function () {
            ...
            if (weaponActions.length > 0) {
                this.addActions(weaponActions, { id: 'monsterWeaponDamage', type: 'system' })
            }
        },
        ...
        buildMonsterDefend: async function () {
            ...
            const defendActions = [{
                ...
                listName: game.i18n.localize('DoD.ui.character-sheet.monsterDefendTooltip'),
