...
export function createActionDialogs(coreModule) {
  return {
    ...
    showLightSourceDialog: async function () {
      const lightSources = [
        {
          ...
          label:
            coreModule.api.Utils.i18n(
              "tokenActionHud.dragonbane.lightSources.lantern"
            ) || "Lantern",
        },
        {
          value: "oilLamp",
          label:
            coreModule.api.Utils.i18n(
              "tokenActionHud.dragonbane.lightSources.oilLamp"
            ) || "Oil Lamp",
        },
        {
          value: "tallowCandle",
          label:
            coreModule.api.Utils.i18n(
              "tokenActionHud.dragonbane.lightSources.tallowCandle"
            ) || "Tallow Candle",
        },
        {
          value: "torch",
          label:
            coreModule.api.Utils.i18n(
              "tokenActionHud.dragonbane.lightSources.torch"
            ) || "Torch",
        },
      ];

      const options = lightSources
        .map(
      ...
      const chooseLabel =
        coreModule.api.Utils.i18n(
          "tokenActionHud.dragonbane.dialog.lightTest.selectSource"
        ) || "Choose a light source:";
      const noteText =
        coreModule.api.Utils.i18n(
          "tokenActionHud.dragonbane.dialog.lightTest.note"
        ) ||
        "Note: All light sources burn for up to one shift of time. Roll of 1 always extinguishes the light.";

      // Get localized table headers
      const tableHeaders = {
        source:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.source"
          ) || "Source",
        radius:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.radius"
          ) || "Radius",
        test:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.test"
          ) || "Test",
        whenToRoll:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.whenToRoll"
          ) || "When to Roll",
        onOne:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.onOne"
          ) || "On 1",
      };
      ...
      const tableContent = {
        afterEachStretch:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.afterEachStretch"
          ) || "After each stretch",
        afterStretchOrCombat:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.afterStretchOrCombat"
          ) || "After stretch or combat",
        afterStretchOrWeapon:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.afterStretchOrWeapon"
          ) || "After stretch or used as weapon",
        refillRelight:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.refillRelight"
          ) || "Refill & relight (action)",
        goesOut:
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.dialog.lightTest.table.goesOut"
          ) || "Goes out",
      };
      ...
      const createStyledLightSourceName = (lightSourceId) => {
        ...
        try {
          ...
          const lightSourceItem = this.actor.items.find(
          ...
          console.warn(
            "Token Action HUD Dragonbane: Error checking light source inventory:",
            error
          );
        }
    ...
    buildSevereInjuryChatContent: async function (
      ...
      const successLabel =
        coreModule.api.Utils.i18n(
          "tokenActionHud.dragonbane.messages.severeInjury.success"
        ) || "Success!";
      const failureLabel =
        coreModule.api.Utils.i18n(
          "tokenActionHud.dragonbane.messages.severeInjury.failure"
        ) || "Failure!";
      ...
      if (isSuccess) {
        const survivesMessage =
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.messages.severeInjury.survivesWithoutInjuries"
          )?.replace("{actor}", actor.name) ||
          `${actor.name} survives without lasting injuries.`;
        ...
        const survivesWithInjuries =
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.messages.severeInjury.survivesWithInjuries"
          )?.replace("{actor}", actor.name) ||
          `${actor.name} survives death, but injuries remain...`;
        const rollSevereInjuryButton =
          coreModule.api.Utils.i18n(
            "tokenActionHud.dragonbane.messages.severeInjury.rollSevereInjury"
          ) || "Roll Severe Injury";
